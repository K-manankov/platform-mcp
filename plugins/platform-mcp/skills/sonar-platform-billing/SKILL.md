...
| права на **каталог** в шлюзе (`authz.map`) | `infra/k8s/platform` (общий) |
...
клиенту, прошедшему `client_credentials` в realm'е контура: подключение
приложения к OpenMeter не требует MR в платформу, `authz.map` для этого не
правят. Отсекает не имя, а Keycloak — публичные клиенты (весь фронт) и
конфиденциальные без `serviceAccountsEnabled` получают 401 `unauthorized_client`
и до правила не доходят.

**Отсюда требование, которое надо проговаривать при подключении: приложение
заводит для OpenMeter свой клиент с `serviceAccountsEnabled`, а не
переиспользует сервисный клиент другой задачи** (SSO, org-sync, CI). Иначе
доступ к биллингу получает всё, что знает тот секрет; отозвать его нельзя, не
сломав ту задачу; и по списку клиентов не видно, кто ходит в OpenMeter. Если
пользователь предлагает взять уже существующий клиент — возразить и объяснить
почему, а не соглашаться.

`catalog` остаётся поимённым — каталог общий (см. выше), и это единственный
класс, ради которого приложению может понадобиться MR в платформу. `other`
(гранты, профили биллинга, `DELETE`) не выдан никому.
...
1. **`KeycloakClient` в `deploy/overlays/<env>/`** (не в `base/`: realm у контуров
   ...
   `client-secret`) — в Vault за ним ходить не надо. Клиент **отдельный, заведённый
   под OpenMeter**: не переиспользовать тот, что у приложения уже есть под другую
   задачу.
...
Всё: `ingest` и `read` работают сразу, как только клиент завёлся. Строка в
`authz.map` платформы нужна, только если приложению понадобится класс `catalog`,
то есть право править общий каталог, — это отдельный разговор и отдельный MR.
