...
4. **MR в `infra/k8s/platform`**: строки `"<client_id>:ingest"` и `"<client_id>:read"`
   в `platform/openmeter-gateway/overlays/{test,prod}/authz.map`. Это единственный
   шаг вне репозитория проекта — права на общий OpenMeter проект себе не выдаёт.
   Без него шлюз ответит Job'у или приложению 403.
