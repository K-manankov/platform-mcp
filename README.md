...
**Cursor.** Plugins → Configure у `platform-mcp`: URL Argo CD и Vault, `PLATFORM_MCP_INSECURE`,
и mount Vault OIDC (`oidc` — обычный вход, `oidc-admin` — полные права для Owner'ов
`infra/k8s`). Дефолты совпадают с кластером `sonar-prod`.
